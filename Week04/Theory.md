...
## Цикли(Loops)
Да предположим, че трябва да напишете програма, която извежда първите 10 цели числа по-големи от числото х. Какво ще направите? Използвайки, математическо мислене стигаме до тук:
```c++
        #include <iostream>
        using namespace std;
...
Красота!  5к заплата! Обаче, шефът се е объркал - искал е да са 10, ама 10 **хиляди**. Е, в такъв случай може да си счупите пръстите от писане на едно и също 10 000 пъти, но може и да сменим подхода. Забелязваме, че е налице **повторение на един и същ оператор, докато не се наруши дадено условие.** А това показва, че имаме **цикличност**.
